...
resource "openstack_networking_port_v2" "test1_ports" {
  for_each   = var.nodes
    name       = "test1-${each.key}-mlops-${var.suffix}"
    network_id = data.openstack_networking_network_v2.test1.id
    security_group_ids = [
...
resource "openstack_compute_instance_v2" "nodes" {
  ...
  network {
    port = openstack_networking_port_v2.test1_ports[each.key].id
  }
...
resource "openstack_networking_floatingip_v2" "floating_ip" {
  ...
  port_id     = openstack_networking_port_v2.test1_ports["node1"].id
}